...
class Itermocil(object):
    ...
    def __init__(self, teamocil_file, here=False, cwd=None, ips=[]):
    ...
    def process_file(self):
        ...
        for num, window in enumerate(self.parsed_config['windows']):
            ...
            if 'panes' in window:
                ...
                if self.new_iterm:
                    self.arrange_panes(num_ips, layout)
                else:
                    self.arrange_panes_old_iterm(num_ips, layout)
                ...
                for pane_num, pane in enumerate(self.ips, start=start_pane):
                    total_pane_count += 1
                    pane_name = pane
                    ...
                    if isinstance(pane, dict):
                        ...
                        pane_commands.append('vssh ' + escaped_command)
...
def main():
    ...
    for idx in range(1, len(args.layout_name)):
        ips.append(args.layout_name[idx])
    ...
    cwd = os.getcwd()
    instance = Itermocil(filepath, here=args.here, cwd=cwd, ips=ips)
